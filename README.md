...
Взаимодействие с GigaChat выполняется с помощью [одноименной библиотеки](https://github.com/ai-forever/gigachat), которая предоставляет интерфейсы для для авторизации и обращения к сервису с помощью [GigaChat API](/ru/gigachat/api/integration).
...
Отличительной особенностью GigaChain является наличие модуля [`gigachat`](#описание-модуля-gigachat), который обеспечивает обмен данными с одноименной нейросетевой моделью с помощью [отдельной библиотеки](https://github.com/ai-forever/gigachat).

> [!NOTE]
> Для работы с GigaChat вам также понадобится [доступ к GigaChat API](https://developers.sber.ru/docs/ru/gigachat/api/integration).
> 
> Сейчас доступ к GigaChat API предоставляется только юридическим лицам и индивидуальным предпринимателям после подписания договора.
...
Модуль [`gigachat`](libs/langchain/langchain/chat_models/gigachat.py) позволяет авторизовать запросы от вашего приложения в GigaChat с помощью [библиотеки GigaChat](https://github.com/ai-forever/gigachat). Модуль поддерживает работу как в синхронном, так и в асинхронном режиме. Кроме этого модуль поддерживает обработку [потоковой передачи токенов](https://developers.sber.ru/docs/ru/gigachat/api/response-token-streaming)[^1].
